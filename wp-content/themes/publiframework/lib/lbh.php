...
class LicenseBoxExternalAPI{
	...
	public function __construct(){ 
		$this->product_id = '0CB9E09C';
		$this->api_url = 'https://license.ideaweb.biz/';
		$this->api_key = '1DF70B28D8E2C8AA3C56';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["ea3a17a2387fe97"])){
				$_SESSION["ea3a17a2387fe97"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["ea3a17a2387fe97"])){
				$get_data = $this->call_api(
					'POST',
